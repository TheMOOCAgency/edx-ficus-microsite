...
select_language = "FR"
try:
    if course_ids[0].index("ENG"):
        select_language = "EN"
except:
    pass

try:
    if course_ids[0].index("DEUCH"):
        select_language = "DE"
except:
    pass
try:
    if course_ids[0].index("IT"):
        select_language = "IT"
except:
    pass
try:
    if course_ids[0].index("ESP"):
        select_language = "ES"
except:
    pass
...
headers = ["ID","First Name", "Last Name","Gender", "Age", "Status","Sector", "Study level","Motivation","Area","Location","Email Address","Registration Date","Last Visit","Number of Visits"]
...
for course_id in course_ids:
    ...
    for i in range(len(course_enrollments)):
        ...
        if str(user.id) not in all_users_data.keys():
            ...
            try:
                user_data["gender"] = json.loads(user.profile.custom_field)['sexe'].capitalize()
            except:
                user_data["gender"] = "n/a"
                
            try:
                user_data["status"] = json.loads(user.profile.custom_field)['statut'].capitalize()
            except:
                user_data["status"] = "n/a"

            try:
                user_data["sector"] = json.loads(user.profile.custom_field)['secteur'].capitalize()
            except:
                user_data["sector"] = "n/a"

            try:
                user_data["study"] = json.loads(user.profile.custom_field)['niveau-etudes'].capitalize()
            except:
                user_data["study"] = "n/a"

            try:
                user_data["motivation"] = json.loads(user.profile.custom_field)['motivation'].capitalize()
            except:
                user_data["motivation"] = "n/a"

            try:
                user_data["area"] = json.loads(user.profile.custom_field)['area'].capitalize()
            except:
                user_data["area"] = "n/a"

            try:
                user_data["location"] = json.loads(user.profile.custom_field)['location'].capitalize()
            except:
                user_data["location"] = "n/a"
        ...
        # SHOULD NOT OCCUR FOR THIS COURSE
        else:
...
for index, user in all_users_data.items():
    ...
    sheet.write(j, 3, user["gender"])
    sheet.write(j, 4, user["age"])
    sheet.write(j, 5, user["status"])
    sheet.write(j, 6, user["sector"])
    sheet.write(j, 7, user["study"])
    sheet.write(j, 8, user["motivation"])
    sheet.write(j, 9, user["area"])
    sheet.write(j, 10, user["location"])
    sheet.write(j, 11, user["email_address"])
    sheet.write(j, 12, user["registration_date"], style1)
    sheet.write(j, 13, user["last_visit"], style1)
    sheet.write(j, 14, user["connections"])
    i=15
    # i=8
    # total_time = 0
...
html = "<html><head></head><body><p>Bonjour,<br/><br/>Vous trouverez en pièce jointe le rapport de donn&eacute;es des MOOCs : "+ course_names_html +"<br/><br/>Bonne r&eacute;ception<br><br></p><p>---------------------------<br><br>Hello,<br/><br/>You will find attached the data report of the MOOCs : "+ course_names_html +"<br/><br/>Good reception<br><br></p><p>---------------------------<br><br>Buenos dias,<br/><br/>Se adjunta el informe de datos de los MOOC : "+ course_names_html +"<br/><br/>Buena recepción<br><br></p><p>---------------------------<br><br>Hallo,<br/><br/>Im Anhang finden Sie den MOOC-Datenbericht : "+ course_names_html +"<br/><br/>Guter Empfang<br><br></p><p>---------------------------<br><br>Buongiorno,<br/><br/>In allegato troverai il rapporto sui dati dei MOOC : "+ course_names_html +"<br/><br/>Buona ricezione<br><br></p></body></html>"
...
for i in range(len(TO_EMAILS)):
   ...
   msg['From'] = fromaddr
   msg['To'] = toaddr
   msg['Subject'] = "Region PACA - " + ' + '.join(course_names)
...
#UPDATE EMAIL ADDRESS AND SCRIPT NAME
#UPDATE EMAIL ADDRESS AND SCRIPT NAME


# sudo -H -u edxapp /edx/bin/python.edxapp /home/edxtma/dev_generic_reports_script/script-nature-for-city.py "valerie.montes@univ-amu.fr;mcvallon@maregionsud.fr;esma.inci@weuplearning.com" "course-v1:nature-for-city-life+NFCL_FR+session2021;course-v1" false false true true

# sudo -H -u edxapp /edx/bin/python.edxapp /home/edxtma/dev_generic_reports_script/script-nature-for-city.py "valerie.montes@univ-amu.fr;mcvallon@maregionsud.fr;esma.inci@weuplearning.com" "course-v1:nature-for-city-life+NFCL_SP_DEUCH+session2021" false false true true

# sudo -H -u edxapp /edx/bin/python.edxapp /home/edxtma/dev_generic_reports_script/script-nature-for-city.py "valerie.montes@univ-amu.fr;mcvallon@maregionsud.fr;esma.inci@weuplearning.com" "course-v1:nature-for-city-life+SPENG2021+Session2021" false false true true

# sudo -H -u edxapp /edx/bin/python.edxapp /home/edxtma/dev_generic_reports_script/script-nature-for-city.py "valerie.montes@univ-amu.fr;mcvallon@maregionsud.fr;esma.inci@weuplearning.com" "course-v1:nature-for-city-life+NFCL_SP_IT+session2021" false false true true

# sudo -H -u edxapp /edx/bin/python.edxapp /home/edxtma/dev_generic_reports_script/script-nature-for-city.py "valerie.montes@univ-amu.fr;mcvallon@maregionsud.fr;esma.inci@weuplearning.com" "course-v1:nature-for-city-life+NFCL_SP_ESP+session2021" false false true true
