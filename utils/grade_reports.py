...
class grade_reports():
    ...
    def return_select_value(self, key,value,kwarg):
        for indice in kwarg:
            ...
            if ensure_user_exist:
                ...
                for n in body:
                    ...
                    #insert register_form mysql value to xls
                        ...
                        _insert_value = self.return_select_value(n,user_profile.get(n),register_form_)
                        sheet_count.write(j, k, _insert_value)
                        k = k + 1

                    #insert certificate_form mongodb value to xls
                    elif n in user_certif_profile.keys():
                        _insert_value = self.return_select_value(n,user_certif_profile.get(n),certificates_form_)
                        sheet_count.write(j, k, _insert_value)
                        k = k + 1
    ...
    def task_generate_xls(self):
        ...
        report_frequence=self.request.get('report_frequence')
        ...
        for enrollment in course_enrollments:
            ...
            #if report_frequence is not None do not include reports wich best grade is older than report_frequence in days
            tma_enrollment=TmaCourseEnrollment.get_enrollment(course_id=self.course_id, user=user)
            if tma_enrollment.best_grade_date == None and (grade_info.percent>0 or tma_enrollment.best_grade>0):
                tma_enrollment.best_grade_date = datetime.now()
                tma_enrollment.save()
            if not report_frequence or not tma_enrollment.best_grade_date or tma_enrollment.best_grade_date.date() < script_reference_date - timedelta(days=report_frequence):
                continue
