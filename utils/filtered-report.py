...
#headers
HEADERS = ["Student ID","Nom","Prénom","Téléphone","Code postal","Username","Email","Diplôme","Spécialité","Année de naissance","Sexe","Agent SNCF","Dossier de candidature déposé","Note","Date inscription"]
...
for i in range(len(course_enrollements)):
    ...
    if comparison_date and comparison_date >= utc.localize(last_report_date) :
        ...
        code_postal=user_profile.get('cp','')
        telephone=user_profile.get('telephone','')
        filiere=user_profile.get('filiere','')
        diplome=user_profile.get('diplome','')
        folder_already_completed=user_profile.get('folder_already_completed',False)
        if folder_already_completed == "true":
            folder_already_completed = "Oui"
        else:
            folder_already_completed = "Non"
        is_already_agent=user_profile.get('is_already_agent',False)
        if is_already_agent == "true":
            is_already_agent = "Oui"
        else:
            is_already_agent = "Non"
        final_grade='n/a'
        try:
        ...
        #insert rows
        primary_rows = [
            student_id,last_name,first_name,telephone,code_postal,username,email,diplome,filiere,year_of_birth,gender,is_already_agent,folder_already_completed,final_grade,register_date
        ]	
...
if users_number >= 1:
    ...
    html = "<html><head></head><body><p>Bonjour,<br/><br/>Il n'y a aucun utilisateur à s'être {} au MOOC \"{}\" ces {} derniers jours.<br/><br/>Bonne réception<br>L'équipe {}<br></p></body></html>".format(based_on_text, course.display_name, str(frequence), org)
